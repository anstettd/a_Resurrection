...
pc.means.w<-site.year.means.wet %>% select("Region","Year","Drought")
...
pc.means.d<-site.year.means.dry %>% select("Region", "Year","Drought")
...
pc.plot.mean <- pc.plot.mean +
  scale_x_continuous(name="PC1 (54.2%)") +
  scale_y_continuous(name="PC2 (20.9%)") +
  theme_grey()+
...
ggsave("6_muti_North_South.pdf", width = 7, height = 6, units = "in")
