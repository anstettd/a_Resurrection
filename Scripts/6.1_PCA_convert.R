...
pc.means.w<-trait.means.w %>% select("ID_Year","Site","Year","Drought")
...
tmeans.d.redu <- trait.means.d %>% select("Experiment_Date","Water_Content","SLA","Stomatal_Conductance","Assimilation","Biomass")
pc.mean.values.d <- scale(tmeans.d.redu , pc1$center, pc1$scale) %*% pc1$rotation 
pc.means.d<-trait.means.d %>% select("ID_Year","Site","Year","Drought")
